...
def stat(root_directories, outputdir):
...
@click.option("--actions", "action_names", default=["stdout"], help="Actions to be performed when triggers fire", multiple=True)
@click.option("--threshhold", default=50, help="Threshold above which actions are triggered")
def missing_files(newscan, oldscan, action_names, threshhold):
    ...
    if newscan is None or oldscan is None:
        ...
        # sort scans by date
        date_sorted = sorted(scan_files, key=lambda x: datetime.strptime(x.split("_")[0], DATEFORMAT), reverse=True)
